"""
교보문고 신규 항목 증분 스크래핑
- temp/신규목록.xlsx 읽어서 신규 ID만 스크래핑
- 기존 db/교보문고_스크래핑결과.xlsx 에 append

사용법:
  python kyobo_scraper_incremental.py
"""
...
def main():
    ...
    total = len(new_df)
    result['total'] = total
    print(f"신규 항목: {total}개 스크래핑 시작\n")

    # 분류
    new_df['시리즈'] = new_df['상품명'].apply(match_series)
    new_df['유형']   = new_df['상품명'].apply(classify_type)

    # 매칭/미매칭 분류
    for _, row in new_df.iterrows():
        if row['시리즈']:
            result['matched'].append((row['상품명'], row['시리즈']))
        else:
            result['unmatched'].append(row['상품명'])
    ...
    for i, row in new_df.iterrows():
        ...
        if res['error']:
            ...
            result['errors'].append(f"{pid} - {res['error']}")
        else:
            print(f"{res['sale_status']} | {res['edition_status']}")
            if '절판' in str(res['sale_status']):
                result['절판'].append(row['상품명'])

        time.sleep(DELAY)

    res_df = pd.DataFrame(results)
    new_df['링크']      = new_df['판매상품ID'].apply(lambda x: f"https://product.kyobobook.co.kr/detail/{str(x).strip()}")
    new_df['표지이미지'] = res_df['cover_image_url'].values
    new_df['판매현황']   = res_df['sale_status'].values
    new_df['개정판여부'] = res_df['edition_status'].values
    new_df['오류']       = res_df['error'].values

    # 기존 DB에 append
    try:
        db_df = pd.read_excel(DB_FILE)
        final = pd.concat([db_df, new_df], ignore_index=True)
    except FileNotFoundError:
        final = new_df

    save_excel(final, DB_FILE)
    print(f"\n완료! → {DB_FILE}  (기존 {len(final)-total}개 + 신규 {total}개 = 총 {len(final)}개)")
    return result
